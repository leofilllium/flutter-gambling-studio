...
| `/store-screenshots` | `.claude/skills/store-screenshots/SKILL.md` | The gambling game's store showcase: a concept triptych around the key moment of a round (one panorama across N panels, with NO text, cut with a ~100px seam allowance so the store's gutter cannot bisect an object) + round frames in a phone frame with captions + feature graphic + icon/emblem (generated AND applied). Continuity gate: the game's real sprites go into the panorama (`triptych --sprite`), its real play field goes in as a cutout built from the shipped symbols or lifted from a captured frame (`boardplate` → `--sprite plate@board`) rather than anything the image model draws, and the panorama goes into the app as its background (`backdrop`) before frames are captured. Panel 1 is drawn as an empty hero berth and the real protagonist is seated into it, occluded by the scene's own foreground. Two sets — 1320×2868 (App Store 6.9″) and 1080×1920 9:16 (Google Play). Compliance gate: no currency symbols, no payout promises. Art from GPT Images 2.0, compositing, grading and typography from `tools/store_compose.py` → `project_zip/` |
