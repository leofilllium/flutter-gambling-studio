...
| `/store-screenshots` | `.claude/skills/store-screenshots/SKILL.md` | The gambling game's store showcase: a lossless, text-free concept panorama plus real round frames in a phone frame, feature graphic and applied icon/emblem. Continuity gate: the game's real sprites and resolving play field become a layout draft, then a high-fidelity reference render produces one integrated picture. Panel 1 leads with a large protagonist whose complete silhouette and held props clear the first seam; real game objects form the cropped bottom frame and fall through the remaining height. The far background is bright and smooth, while saturation and controlled glare are measured. A strict gate blocks dark/hyper-detailed art, weak game-object framing, missing glare, and invalid hero bounds before final panels are written. The panorama goes into the app as its background before final frames are captured. Two sets — 1320×2868 (App Store 6.9″) and 1080×1920 (Google Play). Compliance gate: no currency symbols or payout promises. Output → `project_zip/` |
