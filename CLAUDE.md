...
| `/store-screenshots` | A casino-grade store showcase: mechanic-first tension/reveal/celebration composition in the current game's Design DNA, never a generic neon reskin. Includes a text-free concept panorama sliced into panels + real round frames inside a secondary phone frame + feature graphic + applied app icon and emblem. **Storefront ↔ game continuity is a blocking gate**: the game's real sprites are composited into the panorama, the play field is assembled from the game's own symbol files (`boardplate`) instead of drawn by the image model, and the panorama is wired into the app as its background *before* frames are captured, so the listing and the app show one world. Panel 1 is composed as a berth for the protagonist and the scene's foreground closes back over its feet, so the hero reads as part of the slide rather than inserted onto it. Panels are cut with a ~100px seam allowance so the store's own gutter cannot bisect an object, and all generated art is colour-graded for thumbnail review. Two sets: **1320×2868** for App Store Connect (iPhone 6.9″) and **1080×1920** 9:16 for Google Play. Compliance and gameplay-layout gates → a `.zip` in `project_zip/` |
