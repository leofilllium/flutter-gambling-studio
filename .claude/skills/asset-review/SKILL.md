...
|-----|--------------|----------|
| `NO_ALPHA` | фон вообще не вырезан | FAIL → перегенерировать + `cutout.py` |
| `HARD_EDGE` | бинарная альфа: рваный силуэт «лесенкой» | FAIL по AR6 → перевырезать из исходника; нет исходника — перегенерировать |
| `WHITE_FRINGE` | по краю остался светлый ореол от фона | FAIL по AR6 → то же |
...
- **PNG (Codex)** — перегенерировать через GPT Images 2.0 с ИСПРАВЛЕННЫМ промптом;
  ...
  ассетов. После генерации — вырезание фона через
  `python3 tools/cutout.py <файл> --type sprite` (как в `/autocreate` Фаза 3).
