...
Проверяет полноту и качество Game Design Document для гемблинг игры.
...
- `gambling-game-designer` — полнота и корректность дизайна
- `rtp-mathematician` — проверка математических формул
...
### Шаг 2: gambling-game-designer — Проверка полноты GDD

Агент `gambling-game-designer` проверяет каждый GDD:
...
### Шаг 3: rtp-mathematician — Математическая проверка

Агент `rtp-mathematician` проверяет:
