...
Read `.claude/docs/mobile-phone-contract.md`. This studio ships portrait phone games only;
orientation and iPhone-only device-family configuration are release blockers.
...
### 2.1 — Android phone portrait target

In `android/app/src/main/AndroidManifest.xml`, set the launcher activity to portrait:
...
Do not add tablet resources, adaptive multi-pane navigation, or landscape activity variants.
...
- In `ios/Runner/Info.plist`, keep only `UIInterfaceOrientationPortrait` under
  `UISupportedInterfaceOrientations`. Remove landscape values and the complete
  `UISupportedInterfaceOrientations~ipad` key/array.
- In `ios/Runner.xcodeproj/project.pbxproj`, set `TARGETED_DEVICE_FAMILY = 1;` in **every** build
  configuration. `"1,2"` is forbidden because it enables iPad.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎨 Icons: Android phone (adaptive) + iPhone + Web preview — generated
📱 Target: portrait Android phones + iPhone only — verified
