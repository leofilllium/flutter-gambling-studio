---
name: auto-idea
description: "Автономно генерирует готовую концепцию мини-игры любого жанра (без вопросов пользователю). Выбирает из 30+ архетипов A-AF (гемблинг, пазл, аркада, физика, казуальные, карточные) или придумывает совершенно уникальную механику. Прокручивает Variety Dimensions (setting/mood/палитра/яркость/layout/стиль арта), чтобы игры не повторялись. Включает Design DNA, Layout Archetype, полную карту экранов MVP (10+), UX-поток и craft-уровень токены."
argument-hint: "[--list] | [--archetype A-AF]"
user-invocable: true
...
> Archetype — а не generic "тёмная тема с фиолетовым". Варьируй стиль И композицию между играми.
...
**Y — Bonus Pick "Сундуки Фортуны"**
> Pick-and-click бонус: игрок выбирает из N закрытых объектов (сундуки/коробки), каждый раскрывает множитель или "collect". Фича: dramatic reveal с задержкой, нарастающее напряжение.
...
**AA — Keno "Лотерея Чисел"**
> Игрок выбирает числа на сетке, затем тираж выбирает выигрышные. Совпадения = выплата. Фича: шарики-розыгрыш с физическим подпрыгиванием, подсветка совпадений.

**AB — Sweet Bonanza "Сахарный Взрыв" (scatter-pays слот)**
> Слот без линий: выплата за количество символов (8+ одинаковых где угодно). Tumble/каскад, множитель-бомбы. Фича: сочные взрывы конфет, нарастающий tumble multiplier.

#### 🧩 Puzzle (AC-AD)

**AC — Connect/Flow "Поток"**
> Соедини пары одного цвета трубами/линиями, заполнив всю сетку без пересечений. Фича: жидкостная анимация заполнения трубы, satisfying "щелчок" при завершении пары.

**AD — Merge "Слияние 2048"**
> Свайп сетки, одинаковые сливаются в следующий тир. Цель — дойти до топ-тира. Фича: pop+scale при слиянии, цветовая прогрессия тиров, near-full напряжение.

#### 🏃 Action / Casual (AE-AF)

**AE — Stack Tower "Башня"**
> Тайминг-игра: движущийся блок нужно остановить точно над предыдущим. Промах обрезает блок. Фича: perfect-stack комбо с цветовым сдвигом палитры и нарастающим звуком.

**AF — One-Tap Dodge "Уклонение"**
> Один тап меняет направление/прыжок; уворачивайся от препятствий, собирай очки. Фича: near-miss slow-mo, trail-эффект, нарастающий темп и тряска на скорости.

## Процедурная Уникальная Генерация (Unique Mode)
Если флаг `--archetype` не передан или пользователь явно запросил "уникальную идею", вы **ОБЯЗАНЫ** изобрести **абсолютно новую** механику, не совпадающую с A-AF.
...
1. Прочитайте флаг (если `--list`, выведите таблицу архетипов A-AF + категории).
2. Выберите архетип (по аргументу или случайно, **не повторяя прошлый**).
3. **Прокрутите Variety Dimensions**: выберите setting / mood / palette / brightness /
   Layout Archetype (L1–L6) / стиль арта — непохожие на прошлую игру.
4. Определите жанр выбранного архетипа.
5. Создайте детализированный GDD в `design/gdd/game-concept.md` (включая Design DNA из
   выбранных осей и Layout & Composition Direction).
...
- **2–3 названных реальных хита жанра** (например: слот → Coin Master, Slotomania;
  merge → 2048, Triple Town; раннер → Subway Surfers, Alto's Odyssey)
- Для каждого: что заимствуем **В ОЩУЩЕНИИ** (тайминг остановки барабанов, вес каскада,
  ритм наград, отзывчивость управления) — не в контенте и не в визуале
...
> точки монетизации/телеметрии + (для gambling) compliance. Опиши их КОНКРЕТНО — downstream
...
- **Кривая сложности**: [как растёт; ссылка на level-config.json / difficulty-curve.json]
...
- **Mode 2**: [Endless / Time-Attack / Survival — один из, подходящий жанру]
...
- ⚠️ Для **gambling**: валюта строго ВИРТУАЛЬНАЯ, не покупает исход, не конвертируется в деньги.
...
- **Remote-config ключи** (live-tuning): [сложность, частота рекламы, цены, (gambling) RTP-профиль]

### Compliance (ОБЯЗАТЕЛЬНО заполнить для жанра gambling, иначе — «N/A, не gambling»)
- **Age-gate**: [да/нет] — при первом запуске
- **Disclaimer**: «Только для развлечения. Не на реальные деньги.» — на splash + paytable
- **Responsible-play**: блок в настройках
- **Возрастной рейтинг**: [18+ / по платформе]
```

> Объём держи реалистичным для авто-генерации: контент — это ДАННЫЕ (JSON + параметры в
> GameConfig), а не N рукописных экранов. 24 уровня match-3 = один GameScreen + level-config с
> 24 записями. Это и есть «полная игра» малой ценой контекста.
...
[1 предложение: почему эта композиция подходит этому жанру/механике]
...
Compliance: [N/A | gambling: age-gate + disclaimer 18+]
