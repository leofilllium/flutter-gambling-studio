# Управление контекстом — Flutter Gambling Studio
...
## Gambling-специфичная стратегия

### RTP итерация — быстрые циклы

RTP итерации (веса → симуляция → корректировка) повторяются много раз.
Каждый цикл экономит контекст:

1. rtp-mathematician меняет веса в `rtp-config.json`
2. Запускаем `python3 tools/simulate_rtp.py 100000` (быстро)
3. Читаем результат (маленький вывод)
4. Если в диапазоне 95–97% → записываем → готово

НЕ держи всю математику в разговоре. Она в файле.

### После /balance-check

Результат симуляции записывается в `design/balance/simulation-report.md`.
