...
- Опциональный player-level/XP, если есть в концепте. Источник кривой — `level-config.json` /
  `difficulty-curve.json` (создаёт game-mathematician/game-designer в Фазе 4.5).
...
## Compliance-слой (ОБЯЗАТЕЛЕН для жанра gambling)

Социально-казуальные «гемблинг» игры не пройдут модерацию стора без этого. Если
`design/gdd/game-concept.md` относится к жанру gambling — создайте/обеспечьте:
- **Age-gate** при первом запуске (флаг в SaveService) + «18+ / для развлечения».
- **Disclaimer** «Только для развлечения. Не на реальные деньги. Не предполагает реальных
  выигрышей.» — на splash и в paytable/settings.
- **Responsible-play** блок в настройках (текст + ссылка-заглушка).
- Никакой реальной валюты, реальных выплат, ставок на деньги. Только виртуальные монеты.
Эти строки и флаги — часть SaveService/конфига, UI рисует ui-programmer, но логику флага
(показали ли age-gate) держите вы.
